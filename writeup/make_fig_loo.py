...
Design notes (why it looks like this):
  * Form: a line chart on a log y-axis. The job is "one measured series against two competing
    predictions spanning ten orders of magnitude", and only a log axis lets the independence curve
    and the measured curve appear on the same page at all. That vertical distance IS the finding.
...
MEASURED = "#eb6834"   # what real adapters do
LAW = "#0b0b0b"        # 2/(m+1), distribution-free
...
def main() -> int:
    ...
    # The measured curve and the law lie on top of each other -- that is the result -- so their
    # labels must be pulled apart by hand or they overprint each other into an unreadable smear.
    for y, txt, col in ((law[-1] * 3.4, "law  2/(m+1)", LAW),
                        (meas[-1] / 3.4, "measured", MEASURED),
